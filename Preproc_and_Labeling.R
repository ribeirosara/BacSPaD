...
# Clean biosample accession data by removing repetition identifiers - "merged_df_sel" as the integrated dataset to label 
...
all_metadata_filt <- all_metadata
...
all_metadata_filt <- merge(all_metadata_filt, species_names, by = "genome_id", all.x = TRUE)
# Repeat for genus, family, order, class, and phylum
# Import genus names
genus_names <- read.table("GenusNames_all_filt.csv", sep = ",", dec = ".", header=TRUE, colClasses = c("character", "character"))
